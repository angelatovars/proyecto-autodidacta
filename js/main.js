...
function cargarPerfilSiEsNecesario() {
  ...
  if (paginaActual === "profile.html") {
    ...
    fetch("https://proyecto-autodidacta-backend.onrender.com/profile", {
      method: "GET",
      headers: {
      ...
      .then((data) => {
        if (data) {
          ...
          document.querySelector("#notificacionesUsuario").textContent =
            data.notificaciones ? "Activadas" : "Desactivadas";
        }
      })
      .catch((err) => {
...
function cargarRankingSiEsNecesario() {
  ...
  if (paginaActual === "ranking.html") {
    ...
    fetch("https://proyecto-autodidacta-backend.onrender.com/ranking", {
      method: "GET",
    })
      .then((response) => response.json())
      .then((data) => {
      ...
      .catch((err) => {
        ...
        mostrarMensaje(
          "No se pudo cargar el ranking. Por favor, intenta nuevamente.",
          "error"
        );
      });
  }
...
function gestionarCambioContraseña() {
  const formularioCambioContraseña = document.querySelector(
  ...
  if (formularioCambioContraseña) {
    formularioCambioContraseña.addEventListener("submit", function (e) {
      ...
      const contraseñaActual =
        document.querySelector("#contraseñaActual").value;
      const nuevaContraseña = document.querySelector("#nuevaContraseña").value;

      // Solicitud de cambio de contraseña
      fetch(
        "https://proyecto-autodidacta-backend.onrender.com/auth/change-password",
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("jwt-token")}`,
...
function marcarVisitado() {
  const visitado = JSON.parse(localStorage.getItem("visitado"));
  if (visitado !== null) {
    for (let v of visitado) {
      if (v === "laberinto") {
        console.log("laberinto");
        document.getElementById("juegoLaberinto").style.backgroundColor =
          "#f44336";
      } else if (v === "memoria") {
        document.getElementById("juegoMemoria").style.backgroundColor =
          "#f44336";
      } else if (v === "sopa_letras") {
        document.getElementById("juegoSopaLetras").style.backgroundColor =
          "#f44336";
      } else if (v === "fill_spaces") {
        document.getElementById(
          "actividadLlenarEspacios"
        ).style.backgroundColor = "#f44336";
      } else if (v === "timed_reading") {
        document.getElementById("actividadCronometro").style.backgroundColor =
          "#f44336";
      } else if (v === "words_game") {
        document.getElementById("actividadPalabras").style.backgroundColor =
          "#f44336";
      }
    }
  }
...
function reiniciarJuegos() {
  localStorage.removeItem("visitado");
  window.location.href = "index.html";
}
