  ...
  for (let i = 0; i < sopaLetras.length; i++) {
    for (let j = 0; j < sopaLetras[i].length; j++) {
      ...
      letra.addEventListener("mousedown", (e) =>
        iniciarSeleccion(i, j, letra, e)
      );
      letra.addEventListener("mouseover", (e) =>
        continuarSeleccion(i, j, letra, e)
      );
...
function iniciarTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    timerElement.textContent = `⏱️ Tiempo: ${minutos}:${segundos
      .toString()
      .padStart(2, "0")}`;
...
function continuarSeleccion(fila, columna, elemento, evento) {
  ...
  if (
    deltaFila === 0 ||
    deltaColumna === 0 ||
    Math.abs(deltaFila) === Math.abs(deltaColumna)
  ) {
    limpiarSeleccion();
    seleccionarPalabra(
      seleccionInicial.fila,
...
function verificarPalabra() {
  ...
  if (
    (palabras.includes(palabraFormada) || palabras.includes(palabraReversa)) &&
    !palabrasEncontradas.has(palabraFormada) &&
    !palabrasEncontradas.has(palabraReversa)
  ) {
    const palabraCorrecta = palabras.includes(palabraFormada)
      ? palabraFormada
      : palabraReversa;
    ...
    const palabraElement = document.getElementById(
...
function guardarJuegoBD() {
  const token = localStorage.getItem("token");
  const url = "https://proyecto-autodidacta-backend.onrender.com/api/games";
...
function volverAlMenu(event) {
  ...
  if (!visitado.includes("sopa_letras")) {
