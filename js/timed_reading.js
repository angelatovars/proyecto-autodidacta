...
const niveles = {
  1: {
    texto:
      "El sol brilla en el cielo. Los niños juegan en el parque. Es un día perfecto para disfrutar al aire libre.",
    tiempoMax: 60,
  },
  2: {
    texto:
      "Los animales en la selva viven en armonía. Cada especie tiene su propio papel en el ecosistema.",
    tiempoMax: 50,
  },
  3: {
    texto:
      "La tecnología ha avanzado rápidamente en los últimos años, transformando la forma en que nos comunicamos.",
    tiempoMax: 40,
  },
  4: {
    texto:
      "A lo largo de la historia, el ser humano ha mostrado una increíble capacidad de adaptación.",
    tiempoMax: 30,
  },
  5: {
    texto:
      "El universo es vasto y misterioso. Desde las galaxias distantes hasta los planetas en nuestro propio sistema solar.",
    tiempoMax: 20,
  },
};
...
function verificarFrase() {
  ...
  const vozDetectada = document
    .getElementById("vozDetectada")
...
function guardarPuntajeBD() {
  ...
  const url = "https://proyecto-autodidacta-backend.onrender.com/api/ranking";
...
function guardarActividadBD() {
  const token = localStorage.getItem("token");
  const url =
    "https://proyecto-autodidacta-backend.onrender.com/api/activities";
...
function volverAlMenu(event) {
  let visitado = JSON.parse(localStorage.getItem("visitado")) || [];
  if (!visitado.includes("timed_reading")) {
...
function activarReconocimientoVoz() {
  ...
  recognition.onresult = (event) => {
    const palabraDetectada = event.results[0][0].transcript
      .trim()
      .toUpperCase();
    document.getElementById(
      "vozDetectada"
    ).innerText = `🎤 Palabra Detectada: ${palabraDetectada}`;
...
function resaltarPalabra(palabraDicha) {
  ...
  const textoModificado = palabras
    .map((palabra) =>
      palabra.toUpperCase() === palabraDicha
